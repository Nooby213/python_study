...
# 애초에 코드를 그렇게 안쓰면 되는거 아닐까?
    # 매게변수로 넘겨 받은 데이터가 변경되지 않도록 코드를 쓴다.
    # 그렇게 하라고, 일부러 tuple로 한번 더 변경해줌
def some_func(*args):
    print(args) # (1, 2, 3, 4)
...
#해당 데이터의 값을 변경하거나, 조작하건, 활용하는 용도를 기대
a, *b ,c = [1, 2, 3, 4, 5]
print(b)
print(type(b))
...
- 코드 재사용 가능
- a데이터, b데이터, c데이터 매번 ㄱ 함수를 거쳐야 한다.
...
result3 = map(my_sum, [1, 2])
print(result3)

a = [1, 2, 3]
b = [4, 5, 6]
result4 = map(lambda num1, num2: num1 + num2, a, b)
print(list(result4))

result5 = map(my_sum, a, b)
print(list(result5))
